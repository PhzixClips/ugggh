...
from gui.task_manager import TaskManager, Task
from media.media_processor import MediaProcessor, transcribe_audio_process
...
class MainWindow:
    ...
    def __init__(self):
        ...
        self.active_task_cancel_event = None
    ...
    def run(self):
        self._create_gui()
        # Gracefully shutdown executors on close
    ...
    def _create_status_bar(self):
        ...
        # This frame will hold status messages OR the progress bar
        ...
        # Progress bar and cancel button (initially hidden)
        self.progress_bar = ttk.Progressbar(self.status_message_frame, orient='horizontal', length=200, mode='determinate')
        self.cancel_button = ttk.Button(self.status_message_frame, text='Cancel', command=self._cancel_active_task, state='disabled')
    ...
    def _start_task(self, task: Task, task_name: str):
        """Initiates a background task and updates the UI."""
        if self.active_task_id:
            messagebox.showwarning("Task in Progress", "Another task is already running.")
            return

        self._toggle_controls(enabled=False)
        self._set_status_message(f"Starting: {task_name}...", is_task_status=True)
        self._show_progress_bar(show=True)
        self.progress_bar['value'] = 0

        self.active_task_cancel_event = threading.Event()
        task.kwargs['cancel_event'] = self.active_task_cancel_event

        self.active_task_id = self.task_manager.submit_task(task)
        self.cancel_button.config(state='normal')

    def _cancel_active_task(self):
        """Cancels the currently active task."""
        if self.active_task_id and self.active_task_cancel_event:
            self.active_task_cancel_event.set()
            # The task manager doesn't have a cancel method, the event is enough
            self._set_status_message("Cancelling task...", is_task_status=True)
            self.cancel_button.config(state='disabled')

    def _update_progress(self, data: dict):
        """Callback to update the progress bar and status from a task."""
        if 'line' in data:
            line = data['line']
            # Try to parse yt-dlp progress
            match = re.search(r'\[download\]\s+([0-9.]+)%', line)
            if match:
                percentage = float(match.group(1))
                self.progress_bar['value'] = percentage
                self._set_status_message(f"Downloading... {percentage:.1f}%", is_task_status=True)
            else:
                # show other messages from stdout
                self._set_status_message(line, is_task_status=True)
        elif 'status' in data:
            self._set_status_message(data['status'], is_task_status=True)
        elif 'error' in data:
            self._set_status_message(f"Error: {data['error']}", is_task_status=True)

    def _handle_task_completion(self, task_name: str, result):
        """Handles successful task completion."""
        self.logger.info(f"Task '{task_name}' completed successfully.")
        self._set_status_message(f"{task_name} complete!", is_task_status=False)
        self._reset_task_state()

    def _handle_task_error(self, task_name: str, error):
        """Handles task failure."""
        self.logger.error(f"Task '{task_name}' failed: {error}")
        self._set_status_message(f"{task_name} failed.", is_task_status=False)
        messagebox.showerror("Task Error", f"An error occurred during {task_name}:\n{error}")
        self._reset_task_state()

    def _reset_task_state(self):
        """Resets UI and state after a task is finished or cancelled."""
        self.active_task_id = None
        self.active_task_cancel_event = None
        self._show_progress_bar(False)
        self._toggle_controls(enabled=True)
        self.cancel_button.config(state='disabled')
        if not self.status_job: # Don't clear a newly set status message
            self.root.after(100, self._clear_status_message)


    def _toggle_controls(self, enabled: bool):
        """Enable or disable primary action buttons."""
        state = 'normal' if enabled else 'disabled'

        # Main search and analyze buttons
        if self.generate_button: self.generate_button.config(state=state)
        analyze_button = self.url_frame.winfo_children()[2] # Risky, but works for now
        if analyze_button: analyze_button.config(state=state)

        # Buttons below the results table
        for btn in self.action_buttons.values():
            btn.config(state=state)

    def _show_progress_bar(self, show: bool = True):
        """Shows or hides the progress bar and cancel button."""
        if show:
    ...
    def _clear_status_message(self):
        ...
        for widget in self.status_message_frame.winfo_children():
            # Don't destroy the progress bar or cancel button
            if widget not in [self.progress_bar, self.cancel_button]:
                widget.destroy()
        self.status_message_label = None
        self.undo_button = None

    def _set_status_message(self, message: str, undo_command: Optional[callable] = None, is_task_status: bool = False):
        # Don't show status messages if a progress bar is active
        if self.progress_bar.winfo_ismapped() and not is_task_status:
            return

        self._clear_status_message()

        # Hide progress bar if we're showing a normal status message
        if not is_task_status and self.progress_bar.winfo_ismapped():
            self._show_progress_bar(False)
        ...
        # Task status messages are persistent until the task ends
        if not is_task_status:
    ...
    def _analyze_url(self):
        ...
        self.tab_manager.clear_tab_results(self.tab_manager.get_active_tab_id())

        # Define the analysis task
        def analysis_task(video_id, cancel_event):
            ...
            if not video_entry:
                raise ValueError("Failed to process video data")
            return [video_entry]

        # Completion handler
        def on_complete(results):
            self._handle_task_completion("URL Analysis", results)
            active_tab_id = self.tab_manager.get_active_tab_id()
            if results:
                for video in results:
                    self.tab_manager.add_result_to_tab(active_tab_id, video)
                video_title = results[0].get('title', 'Unknown')[:30]
                self.tab_manager.update_tab_status(active_tab_id, f"✓ {video_title}...", 'complete')
                show_toast(self.root, f"Analysis complete: {video_title}")
            else:
                self.tab_manager.update_tab_status(active_tab_id, "No data", 'error')

        task = Task(
            target=analysis_task,
            args=(video_id,),
            on_complete=on_complete,
            on_error=lambda e: self._handle_task_error("URL Analysis", e),
            task_type='thread' # Network I/O is a thread task
        )
        self._start_task(task, "URL Analysis")
    ...
    def _start_search(self):
        ...
        try: max_duration = int(self.max_duration_entry.get()) if self.max_duration_entry.get() else None
        except ValueError: max_duration = None
        try: desired_count = int(self.count_entry.get().strip() or "0")
        except ValueError: desired_count = 0
        try: min_vph = float(self.vph_entry.get() or 0)
        except ValueError: min_vph = 0.0

        self.search_engine.reset_search()

        # Progress callback for search
        def search_progress_callback(found_count, total_target, status_text=None):
            if status_text:
                self._update_progress({'status': status_text})
            if found_count >= 0:
                progress_text = (
                    f"Found: {found_count}/{total_target} | "
                    f"API cost: {self.search_engine.api_client.api_call_count}/100"
                )
                self._set_status_message(progress_text, is_task_status=True)
                # Simple progress bar based on API calls
                progress_percentage = min((self.search_engine.api_client.api_call_count / 100) * 100, 100)
                self.progress_bar['value'] = progress_percentage

        # Completion handler
        def on_complete(results):
            self._handle_task_completion("Search", results)
            active_tab_id = self.tab_manager.get_active_tab_id()
            for video in results:
                self.tab_manager.add_result_to_tab(active_tab_id, video)

            status = f"{len(results)} results"
            if self.active_task_cancel_event and self.active_task_cancel_event.is_set():
                status = f"{len(results)} partial"

            self.tab_manager.update_tab_status(active_tab_id, status, 'complete')
            show_toast(self.root, f"Search complete. Found {len(results)} videos.")

        task = Task(
            target=self.search_engine.smart_search_fill,
            args=(query, desired_count, published_after, min_vph, max_duration, search_progress_callback),
            on_complete=on_complete,
            on_error=lambda e: self._handle_task_error("Search", e),
            task_type='thread'
        )
        self._start_task(task, "YouTube Search")
    ...
    def _download_video(self):
        ...
        def on_complete(success):
            self._handle_task_completion("Download", success)
            if success:
                show_toast(self.root, f"Download complete: {title[:30]}...")
                try:
                    ...
                    self.logger.error(f'Post-download action error: {e}')
            else:
                if not (self.active_task_cancel_event and self.active_task_cancel_event.is_set()):
                    messagebox.showerror('Download Error', 'Failed to download video.')

        task = Task(
            target=self.media_processor.download_video,
            args=(video_id, url, output_path),
            kwargs={'progress_callback': self._update_progress},
            on_complete=on_complete,
            on_error=lambda e: self._handle_task_error("Download", e),
            task_type='thread'
        )
        self._start_task(task, "Video Download")
    ...
    def _transcribe_video(self):
        ...
        # This is a multi-step task: download then transcribe
        def transcription_flow(cancel_event, progress_callback):
            # 1. Download audio
            progress_callback({'status': 'Downloading audio for transcription...'})
            audio_path = self.media_processor.download_audio_only(
                video_id, url, AUDIO_CLIPS_PATH, progress_callback, cancel_event
            )
            if not audio_path:
                raise Exception("Failed to download audio.")
            if cancel_event.is_set(): return None

            # 2. Transcribe audio (in a separate process)
            progress_callback({'status': 'Submitting for transcription...'})

            # Since this flow is already in a thread, we can block here waiting for the process
            # We use the task manager's process pool directly
            future = self.task_manager.process_executor.submit(transcribe_audio_process, str(audio_path))
            transcript = future.result() # This will block the worker thread, but not the UI

            return transcript

        def on_complete(transcript):
            self._handle_task_completion("Transcription", transcript)
            if transcript:
                clean_title = title.replace('🟢 ','').replace('🔴 ','').strip()
        ...
        task = Task(
            target=transcription_flow,
            args=(),
            kwargs={'progress_callback': self._update_progress},
            on_complete=on_complete,
            on_error=lambda e: self._handle_task_error("Transcription", e),
            task_type='thread' # The flow controller runs in a thread
        )
        self._start_task(task, "Transcription")
    ...
    def _find_raw_source(self):
        ...
        def on_complete(results):
            self._handle_task_completion("Raw Source Processing", results)
            if results and results.get('success'):
                ...
                if not (self.active_task_cancel_event and self.active_task_cancel_event.is_set()):
                    messagebox.showerror('Error', 'Failed to process video for raw source.')

        task = Task(
            target=self.media_processor.process_video_for_analysis,
            args=(video_id, url, base_path),
            kwargs={'progress_callback': self._update_progress},
            on_complete=on_complete,
            on_error=lambda e: self._handle_task_error("Raw Source Processing", e),
            task_type='thread'
        )
        self._start_task(task, "Raw Source Processing")
    ...
    def _download_transcript_async(self, video_id: str, callback: Optional[callable] = None):
        """
        Downloads a transcript in the background. This is typically used for non-interactive
        downloads, like from the save dialog.
        """
        self.logger.info(f"Starting async transcript download for {video_id}")

        def download_and_transcribe_flow(cancel_event, **kwargs):
            url = f'https://www.youtube.com/watch?v={video_id}'
            audio_path = self.media_processor.download_audio_only(video_id, url, AUDIO_CLIPS_PATH, cancel_event=cancel_event)
            if not audio_path or cancel_event.is_set():
                raise Exception("Audio download failed or was cancelled.")
            ...
            # Manually save the transcript, as this flow is non-interactive
            self.transcripts_manager.save({
                'video_id': video_id,
                'text': transcript,
        ...
        def on_complete(transcript):
            self.logger.info(f"Async transcript download complete for {video_id}")
            if callback:
                self.root.after(0, callback)

        def on_error(e):
            self.logger.error(f"Async transcript download failed for {video_id}: {e}")

        # This task runs in the background and doesn't affect the main UI state
        task = Task(
            target=download_and_transcribe_flow,
            on_complete=on_complete,
            on_error=on_error,
            task_type='thread'
        )
        # We create a cancel event but don't tie it to the main UI
        cancel_event = threading.Event()
        task.kwargs['cancel_event'] = cancel_event
